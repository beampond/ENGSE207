// TaskBoard — Frontend (shared across week 3-5)
const API = '/api/tasks';

async function load() {
  const [tasksRes, statsRes] = await Promise.all([fetch(API), fetch('/api/stats')]);
...
document.getElementById('addForm').addEventListener('submit', async e => {
  ...
  await fetch(API, {
    ...
    body: JSON.stringify({ title, description: document.getElementById('fDesc').value.trim(), status: document.getElementById('fStatus').value })
  });
...
document.getElementById('btnCancel').onclick = () => document.getElementById('overlay').classList.remove('active');
